...
def get_assigned_users_from_tasks(shot_id):
    ...
    shot_details = shotgrid_connection.find_one(
        "Shot",
        [["id", "is", shot_id]],
        ["code", "project.Project.name", "sg_sequence.Sequence.code"]
    )
    ...
    tasks = shotgrid_connection.find(
        ...
        ["task_assignees", "step.Step.short_name"]  
    )
    ...
    for task in tasks:
        ...
        if task_assignees:
            for assignee in task_assignees:
                user_data = shotgrid_connection.find_one(
                    "HumanUser", [["id", "is", assignee["id"]]], ["email"]
                )
                if user_data:
...
def get_assigned_users_from_asset_tasks(asset_id):
    ...
    asset_details = shotgrid_connection.find_one(
        "Asset",
        [["id", "is", asset_id]],
    ...
    if request.method == 'POST':
        if request.json:
            ...
            try:
                ...
                elif entity_type == "Note" and event_type == "Shotgun_Note_New" and operation == "create":
                    return handle_note_event(event_data)
                elif entity_type == "Reply" and event_type == "Shotgun_Reply_New" and operation == "create":
                    return handle_reply_event(event_data)
                elif entity_type == "Task" and event_type == "Shotgun_Task_Change" and operation == "update":
...
def handle_task_assignment_event(event_data):
    ...
    task_details = shotgrid_connection.find_one(
        "Task",
        [["id", "is", entity_id]],
        ["entity", "step.Step.short_name"]
    )
...
def handle_reply_event(event_data):
    ...
    if note_links:
        ...
        if linked_entity["type"] == "Shot":
            ...
            assigned_users_by_step, shot_name, sequence_name, project_name = get_assigned_users_from_tasks(linked_shot_id)

            if assigned_users_by_step:
                message_content = (
                    f"A new Reply has been created:\n{reply_content}\n"
                    f"Related Note: {note_content}"
                )

                send_message_to_assigned_users(assigned_users_by_step, shot_name, sequence_name, project_name, message_content)
                return 'success', 200
            else:
    ...
    if slack_user_id:
        message_content = (
            f"A new Reply has been created by you:\n{reply_content}\n"
            f"Related Note: {note_content}"
        )
        send_slack_message(slack_user_id, message_content)
        return 'success', 200
